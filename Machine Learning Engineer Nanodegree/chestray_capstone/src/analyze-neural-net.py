...
for index, label in enumerate(prediction_labels):
    ...
    #plt.matshow(confusion_matrix(test_Y[:,index], round_pred[:, index]))

# Generate the ROC Curve with Area Under the Curve metrics
Visuals.draw_roc_auc("ChestR VGG19 CNN ROC-AUC", prediction_labels, test_Y, pred_Y)
#plt.show()
plt.savefig(os.path.join(definitions.OUTPUT_GENERATED_IMAGES_FOLDER, "chestr-vgg19_roc_auc.jpg"))
...
for (idx, c_ax) in zip(sickest_idx, m_axs.flatten()):
    ...
    stat_str = [n_class[:6] for n_class, n_score in zip(prediction_labels, test_Y[idx]) if n_score>0.5]
    pred_str = ['%s:%2.0f%%' % (n_class[:4], p_score*100)  for n_class, n_score, p_score in zip(prediction_labels, test_Y[idx], pred_Y[idx]) 
        if (n_score>0.5) or (p_score>0.5)]
    c_ax.set_title('Dx: '+', '.join(stat_str)+'\nPDx: '+', '.join(pred_str))
    c_ax.axis('off')

#plt.show()
plt.savefig(os.path.join(definitions.OUTPUT_GENERATED_IMAGES_FOLDER, "chestr-vgg19_sample-images-pred-view.jpg"))
